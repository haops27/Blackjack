...
public class MainController {
    ...
    private static List<Card> allDiscarded = new ArrayList<>();
    ...
    private void resetForNewRound() {
    	   System.out.println("=== resetForNewRound ===");
    	    System.out.println("Deck before discarding: " 
    	        + game.getDeck().size() 
    	        + ", discardPile=" 
    	        + game.getDeck().discardSize());

    	    // STEP 1: move each card into the Deck’s discard pile
    	    allDiscarded.clear();  // clear controller’s list
    	    // dealer’s cards
    	    for (Card c : game.getDealer().getHand().getCards()) {
    	        game.getDeck().discard(c);           // add to deck.discardPile
    	        allDiscarded.add(c);                 // keep local history
    	    }
    	    // each player’s cards
    	    ...
    	    System.out.println("After discarding: deck=" 
    	        + game.getDeck().size() 
    	        + ", discardPile=" 
    	        + game.getDeck().discardSize());

    	    // STEP 2: if you want to reshuffle *all* discard back in now, do it here:
    	    if (game.getDeck().discardSize() > 0) {
    	        boolean didShuffle = game.getDeck().reshuffle();
    	        System.out.println("Reshuffle called? " + didShuffle 
    	            + " → deck=" 
    	            + game.getDeck().size() 
    	            + ", discardPile=" 
    	            + game.getDeck().discardSize());
    ...
    private void showDealer() {
        ...
        if (!dealerRevealed) {
            ...
            // only show the hidden back‐card if there's a second card in hand
            if (cards.size() > 1) {
                dealerCards.getChildren().add(
                    makeCardView(BACK_IMAGE_PATH, 1)
                );
            }

            dealerSumLabel.setText("?");

            // layout width for either 1 or 2 cards
            int displayCount = Math.max(1, cards.size() > 1 ? 2 : cards.size());
            dealerCards.setPrefWidth(CARD_W + (displayCount - 1) * CARD_OFF);
            dealerSumLabel.setVisible(true);
        }
        else {
            // fully revealed: show every card
            ...
            dealerCards.setPrefWidth(CARD_W + (cards.size() - 1) * CARD_OFF);
